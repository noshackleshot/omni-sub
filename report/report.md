...
Я запустил предобученную модель auto_avsr (base, 28% WER на бенчмарке LRS3) напрямую на 49 новых клипах. Пайплайн: MediaPipe → обрезка рта 96×96 → Conformer encoder → beam search (beam_size=40, ctc_weight=0.1).
...
$$\text{score} = (1 - \lambda) \cdot \text{VSR}_{\text{norm}} + \lambda \cdot \text{LM}_{\text{norm}} - \text{rep\_penalty}$$
...
![Архитектура пайплайна](figures/pipeline.jpeg)
...
![Reranking гипотез](figures/reranking.jpeg)
...
**Модель:** auto_avsr Conformer — end-to-end VSR архитектура. Encoder: 12 блоков Conformer (768 dim, multi-head attention + convolution). Decoder: 6 блоков Transformer. Совместное CTC+Attention декодирование. ~250M параметров (large).
...
$$\text{score}_i = (1 - \lambda) \cdot \hat{s}^{\text{VSR}}_i + \lambda \cdot \hat{s}^{\text{LM}}_i - \text{rep}(h_i)$$
